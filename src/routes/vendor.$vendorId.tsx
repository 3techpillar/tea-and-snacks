...
function VendorDashboard() {
  ...
  const ordersQuery = useQuery({
    queryKey,
    queryFn: () => getVendorOrdersFn({ data: { vendorId } }),
    enabled: hasAccess,
  ...
  const statusMutation = useMutation({
    mutationFn: (input: { orderId: string; status: OrderStatus }) =>
      updateOrderStatusFn({ data: { vendorId, ...input } }),
    onSuccess: onMutationSuccess,
  });
  const confirmMutation = useMutation({
    mutationFn: (orderId: string) =>
      confirmPaymentFn({ data: { vendorId, orderId } }),
    onSuccess: onMutationSuccess,
  });
  const rejectMutation = useMutation({
    mutationFn: (orderId: string) =>
      rejectPaymentFn({ data: { vendorId, orderId } }),
    onSuccess: onMutationSuccess,
  });
  const noteMutation = useMutation({
    mutationFn: (input: { orderId: string; note: string }) =>
      addVendorNoteFn({ data: { vendorId, ...input } }),
    onSuccess: onMutationSuccess,
  });
